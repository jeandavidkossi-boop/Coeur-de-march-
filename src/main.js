...
if (panier.length > 0) {
    ...
    return div.innerHTML;
        ...
        if (boutiqueRecherche) {
            setTimeout(() => {
                const vendeurTrouve = vendeursVip ? vendeursVip.find(v => String(v.whatsapp) === String(boutiqueRecherche)) : null;
        ...
        function filtrerVIP(telVendeur, nomBoutique, imageCouverture = '') {
...
function partager() {
    ...
    try {
        ...
        let numeroWa = String(telWhatsApp).replace(/\s+/g, '').replace('+', '');
